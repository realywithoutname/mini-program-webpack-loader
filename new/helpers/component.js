 ...
 */
module.exports.resolveFilesForPlugin = async function (resolver, request, componentSet) {
  const content = require(request)

  const add2Set = (componentPath) => componentSet.add(componentPath)

  return await componentFiles(resolver, request, content, add2Set, add2Set)
}

/**
 * 提供给 loader 使用来获取自定义组件或者页面依赖的自定义组件文件列表
 */
module.exports.resolveFilesForLoader = async function (resolver, request, content, getRelativePath, componentSet) {
  /**
   * 写回组件的相对路径
  ...
  let setRelComponent = (componentPath, key, obj) => {
    componentSet.add(componentPath) // 最后一起删除
