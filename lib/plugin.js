...
class MiniPlugin {
  ...
  entrys(entry) {
    ...
    let entrys = [this.getPagePaths(this.mainContext, 'project.config'), this.options.extfile ? this.getPagePaths(this.mainContext, 'ext') : [],
    // 打包主入口对应的 js 文件
  ...
  getPagesEntry(context, entry) {
    ...
    let pageFiles = [].concat(pages.map(page => this.getPagePaths(context, page)), subPackages && subPackages.map(({
      root,
      pages
    }) => pages.map(page => this.getPagePaths(context, join(root, page)))) || []);
