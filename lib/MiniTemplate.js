...
module.exports = class MiniTemplate {
  ...
  setRender(bootstrapSource, chunk, hash, moduleTemplate, dependencyTemplates) {
    try {
      ...
      source.add(mainTemplate.hooks.modules.call(new RawSource(''), chunk, hash, moduleTemplate, dependencyTemplates));
  ...
  setLocalVars(source, chunk, hash) {
    try {
      ...
      return Template.asString(['// 使用缓存中加载过的模块作为默认加载过的模块，否则公共模块会被调用多次', `var installedModules = ${globalBrage}.__installedModules = ${globalBrage}.__installedModules || {}`]);
    } catch (e) {
      console.log(e);
    }
  }
