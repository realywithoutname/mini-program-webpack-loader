...
class MiniPlugin extends MiniProgam {
  ...
  apply(compiler) {
    ...
    let resourcePaths = new Set(this.entryContexts.concat(this.options.resources));
  ...
  setCacheGroup() {
    ...
    function fn(module, packageRoot) {
      ...
      const notOnlyInThisPackage = module._usedModules && Array.from(module._usedModules).some(moduleName => !reg.test(moduleName));
